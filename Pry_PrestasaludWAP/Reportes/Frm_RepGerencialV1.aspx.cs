...
{
    public partial class Frm_RepGerencialV1 : System.Web.UI.Page
    {
        #region Variables
        ...
        private void FunGenerarReporte(DataTable dtp)
        {
            try
            {
                if (rdbHoras.Checked) reporte = "RptGerencialDoctorHoras.rdlc";
                if (rdbCiudad.Checked) reporte = "RptGerencialDoctorCiudad.rdlc";
                if (rdbPrestadora.Checked) reporte = "RptGerencialDoctorPrestadora.rdlc";
                if (rdbEspecialidad.Checked) reporte = "RptGerencialDoctorEspecialidad.rdlc";
                if (rdbGenero.Checked) reporte = "RptGerencialDoctorGenero.rdlc";
                if (rdbProducto.Checked) reporte = "RptGerencialDoctorProducto.rdlc";
                rptGerencial.Reset();
        ...
        private DataTable funGetDoctorDetail(int CodigoCita)
        {
            ...
            dt = new Conexion(2, "").funConsultarSqls("sp_ReporteGerencialDetalle", objparam);
            return dt.Tables[0]; 
        }
        ...
        private DataTable funGetCiudadDetail(string fechadesde, string fechahasta, string tipoagenda, string tipocliente, int ciucod, int codamp)
        {
            ...
            objparam[4] = ciucod;
            objparam[5] = codamp;
            dt = new Conexion(2, "").funConsultarSqls("sp_ReporteCiudadDetalle", objparam);
            return dt.Tables[0];
        ...
        protected void btnProcesar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (int.Parse(ViewState["Procesar"].ToString()) == 1)
                {
                    ...
                    {
                        citastatus = "where CI.cita_estatuscita in(''A'',''S'',''T'') and ";

                        if (ddlTipoCliente.SelectedValue != "S") citastatus += " CI.cita_tipocliente=''" + ddlTipoCliente.SelectedValue + "'' and ";
                    }
                    if (ddlTipoAgenda.SelectedValue == "C")
                    {
                        citastatus = "where CI.cita_estatuscita in(''C'') and ";
                        if (ddlTipoCliente.SelectedValue != "S") citastatus += " CI.cita_tipocliente=''" + ddlTipoCliente.SelectedValue + "'' and ";
                    }
                    sqlContador = "select COUNT(*) from Expert_CITAMEDICA CI (nolock) ";
                    sqlContador += "INNER join Expert_PRESTADORA PR ON PR.PRES_COD = CI.PRES_COD ";
                    sqlContador += "INNER JOIN Expert_ESPECIPRESTADORA ES ON ES.EXPR_CODIGO=CI.EXPR_CODIGO ";
                    sqlContador += "INNER JOIN Expert_MEDICO ME ON ME.MEDI_COD=CI.MEDI_COD ";
                    sqlContador += "INNER JOIN Expert_TITULAR TI ON TI.TITU_CODIGO = CI.cita_titucodigo ";
                    sqlContador += "INNER JOIN Expert_PRODUCTOS PD ON PD.PROD_CODIGO = TI.PROD_CODIGO ";
                    citastatus += "CONVERT(date,CI.cita_fechacreacion,101) between convert(date," + "''" + txtFechaDesde.Text + "'',101)" + " and convert(date," + "''" + txtFechaHasta.Text + "'',101) and ";
                    ...
                    if (rdbHoras.Checked)
                    {
                        sqlFormat = "select Definicion = DATEPART(hour,CI.cita_fechacreacion),IdCodigo = DATEPART(hour,CI.cita_fechacreacion),";
                        sqlFormat += "TotalCitas = COUNT(*) from Expert_CITAMEDICA CI (nolock) ";
                        sqlFormat += "INNER JOIN Expert_TITULAR TI ON TI.TITU_CODIGO = CI.cita_titucodigo ";
                        sqlFormat += citastatus;
                        sqlFormat += "GROUP BY DATEPART(hour,CI.cita_fechacreacion) ORDER BY COUNT(*) desc";
                        tipoReport = 0;
                    }
                    if (rdbCiudad.Checked)
                    {
                        sqlFormat = "select Definicion = CU.ciud_nombre,IdCodigo = CU.CIUD_COD,TotalCitas = COUNT(*) from Expert_CITAMEDICA CI (nolock) ";
                        sqlFormat += "INNER join Expert_PRESTADORA PR ON PR.PRES_COD = CI.PRES_COD ";
                    ...
                    {
                        sqlFormat = "select Definicion = PR.pres_nombre,IdCodigo = PR.PRES_COD,TotalCitas = COUNT(*) from Expert_CITAMEDICA CI (nolock) ";
                        sqlFormat += "INNER join Expert_PRESTADORA PR ON PR.PRES_COD = CI.PRES_COD ";
                        sqlFormat += "INNER JOIN Expert_TITULAR TI ON TI.TITU_CODIGO = CI.cita_titucodigo ";
                        sqlFormat += citastatus;
                        sqlFormat += "GROUP BY PR.pres_nombre,PR.PRES_COD ORDER BY COUNT(*) desc";
                        tipoReport = 2;
                    }
                    if (rdbEspecialidad.Checked)
                    {
                        sqlFormat = "select Definicion = ES.expr_especialidad,IdCodigo = ES.EXPR_CODIGO,TotalCitas = COUNT(*) ";
                        sqlFormat += "from Expert_CITAMEDICA CI (nolock) ";
                        sqlFormat += "INNER JOIN Expert_ESPECIPRESTADORA ES ON ES.EXPR_CODIGO=CI.EXPR_CODIGO ";
                        sqlFormat += "INNER JOIN Expert_TITULAR TI ON TI.TITU_CODIGO = CI.cita_titucodigo ";
                        sqlFormat += citastatus;
                        sqlFormat += "GROUP BY ES.expr_especialidad,ES.EXPR_CODIGO ORDER BY COUNT(*) desc";
                        tipoReport = 3;
                    }
                    if (rdbGenero.Checked)
                    {
                        sqlFormat = "select Definicion = case PE.pers_genero when ''M'' then ''MASCULINO'' else ''FEMENINO'' end,";
                        sqlFormat += "IdCodigo = PE.pers_genero,TotalCitas = COUNT(*) from Expert_CITAMEDICA CI (nolock) ";
                        sqlFormat += "INNER JOIN Expert_TITULAR TI ON TI.TITU_CODIGO = CI.cita_titucodigo ";
                        sqlFormat += "INNER JOIN Expert_PERSONA PE ON PE.PERS_CODIGO = TI.PERS_CODIGO ";
                        sqlFormat += citastatus;
