...
{
    public partial class FrmAtencionExamen : Page
    {
        #region Variables
        ...
        protected void BtnGrabar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dts.Tables[0].Rows[0][0].ToString() == "OK-Update")
                    Response.Redirect("FrmPrestadoraExamenAdmin.aspx?MensajeRetornado=Guardado con Éxito", true);
                else
                    Response.Redirect("FrmPrestadoraExamenAdmin.aspx?MensajeRetornado=Si registro Existe un Error", true);
            }
            catch (Exception ex)
            {
                Lblerror.Text = ex.ToString();
        ...
        {
            Response.Redirect("FrmPrestadorExamenAdmin.aspx", true);
        }
        #endregion
    }
}