...
{
    public partial class FrmRevisionExamen : Page
    {
        #region Variables
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    Lbltitulo.Text = "Registro Exámenes";
                }
                catch (Exception ex)
                {
        ...
        protected void BtnGrabar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new Funciones().funShowJSMessage("Seleccione Resultado de la Revisión..!", this);
                    return;
                }

                if (string.IsNullOrEmpty(TxtObservacion.Text.Trim()))
                {
                    new Funciones().funShowJSMessage("Ingrese Observación de la Revisión..!", this);
                    return;
                }

                Array.Resize(ref objparam, 11);
                if (DdlResultado.SelectedValue == "EPR")
                {
                    objparam[0] = 27;
                    ...
                    objparam[4] = "";
                    objparam[5] = "";
                    objparam[6] = int.Parse(ViewState["CodigoEXSO"].ToString());
                ...
                objparam[0] = 25;
                objparam[1] = TxtObservacion.Text.Trim().ToUpper();
                objparam[2] = DdlResultado.SelectedValue;
                ...
                objparam[10] = 0;
                dts = new Conexion(2, "").FunConsultaDatos1(objparam);
                Response.Redirect("FrmExamenRealizadoAdmin.aspx?MensajeRetornado='Guardado con Éxito'", true);
            }
            catch (Exception ex)
            {
                Lblerror.Text = ex.ToString();
        ...
        {
            Response.Redirect("FrmExamenRealizadoAdmin.aspx", true);
        }
        #endregion
    }
}